...
My personal Twitter dev account (!!!)
``` {r}
library(rtweet)
...
# Trying topic modelling with #stopthesteal
...
aggregate function
```{r}
tweets_byusers <- aggregate(x = tweets_stopthesteal$text, by = list(tweets_stopthesteal$screen_name), paste, collapse=". ")

colnames(tweets_byusers) <- c("screen_name", "text")
```

convert text to corpus
```{r}
tweets_byusers_corpus <- iconv(tweets_byusers$text)
...
corpus <- tm_map(corpus, removeWords,c("stopthesteal")) 
...
looking at first 5 rows/columns of the DTM
```{r}
inspect(dtm[1:5, 1:5])  
```

25 most frequent terms
```{r}
...
topic model initial parameters
```{r}
burnin <- 4000
iter <- 2000
thin <- 500
...
specify number of topics - trying 5
```{r}
k <- 5 #find 5 topics
